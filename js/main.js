! function ()
{
	...
	function t(e, t)
	{
		e.classList.add("ty"), t.style.maxHeight = t.scrollHeight + "px"
	}

	function n(e, t)
	{
		e.classList.remove("ty"), t.style.maxHeight = null
	}
	if (e.length > 0)
		for (let i = 0; i < e.length; i++)
		{
			...
			a.classList.contains("ty") && t(a, l), s.addEventListener("click", function ()
			{
...
function ()
{
	...
	function n(e, t)
	{
		...
		n.classList.remove("ty"), a.classList.remove("ty"), i ? (i.classList.add("ty"), l.classList.add("ty")) : "next" === t ? (e.firstElementChild.classList.add("ty"), e.parentNode.getElementsByClassName("tz")[0].firstElementChild.classList.add("ty")) : (e.lastElementChild.classList.add("ty"), e.parentNode.getElementsByClassName("tz")[0].lastElementChild.classList.add("ty"))
	}

	function i(e, t)
	{
		let n, i = 0;
		for (let e = 0; e < t.length; e++) t[0].parentNode.style.minHeight = i + "px", t[e].classList.add("b"), n = t[e].offsetHeight, t[e].classList.remove("b"), n > i && (i = n);
	...
	if (t.length > 0)
		for (let a = 0; a < t.length; a++)
		{
			...
			d.className = "tz", l.parentNode.insertBefore(d, l.nextSibling);
			for (let e = 0; e < c.length; e++)
			{
				c[e].setAttribute("data-carousel", e), c[e].classList.contains("ty") && (r = e);
				let t = document.createElement("tbuttonn");
				t.className = "ne", t.setAttribute("data-bullet", e), l.parentNode.getElementsByClassName("tz")[0].appendChild(t)
			}
			c[r].classList.add("ty");
			let u = l.parentNode.getElementsByClassName("ne");
			u[r].classList.add("ty"), i(0, c), window.addEventListener("resize", function ()
			{
				i(0, c)
			});
			let m = !1;
			...
			for (let e = 0; e < u.length; e++)
			{
				...
				{
					if (e.preventDefault(), t.classList.contains("ty")) return;
					for (let e = 0; e < u.length; e++) u[e].classList.remove("ty");
					for (let e = 0; e < c.length; e++) c[e].classList.remove("ty");
					let n = this.getAttribute("data-bullet");
					c[n].classList.add("ty"), this.classList.add("ty"), s(m)
				})
			}
			e.init(l, function (e)
			{
...
function ()
{
	...
	{
		document.body.classList.toggle("td"), t.classList.toggle("ty"), t.style.maxHeight ? t.style.maxHeight = null : t.style.maxHeight = t.scrollHeight + "px", "true" === this.getAttribute("aria-expanded") ? this.setAttribute("aria-expanded", "false") : this.setAttribute("aria-expanded", "true")
	}), document.addEventListener("click", function (n)
	{
		n.target === t || n.target === e || t.contains(n.target) || (document.body.classList.remove("td"), t.classList.remove("ty"), t.style.maxHeight = null, e.setAttribute("aria-expanded", "false"))
	}))
}(),
function ()
{
	...
	function n()
	{
		...
		for (let t = 0; t < e.length; t++) e[t].classList.remove("ty")
	}
	if (e.length > 0 && t.length > 0)
		for (let e = 0; e < t.length; e++)
		{
			...
			{
				var t;
				e.preventDefault(), n.hasAttribute("aria-controls") && (t = i) && (document.body.classList.add("modal-is-active"), t.classList.add("ty"))
			}))
		}
	document.addEventListener("click", function (e)
	{
		(e.target.classList.contains("tw") || e.target.classList.contains("modal-close-trigger")) && (e.preventDefault(), n())
...
function ()
{
	...
	if (e.length > 0)
		for (let n = 0; n < e.length; n++)
		{
			...
			s.data = JSON.parse(s.el.getAttribute("data-price-input")), s.currentValEl = i.querySelector(".ie"), s.thumbSize = parseInt(window.getComputedStyle(s.currentValEl).getPropertyValue("--thumb-size"), 10);
			...
			for (let e = 0; e < a.length; e++)
			{
				const t = a[e],
					n = {};
				n.currency = t.parentNode.querySelector(".pricing-item-price-currency"), n.amount = t.parentNode.querySelector(".pricing-item-price-amount"), n.after = t.parentNode.querySelector(".pricing-item-price-after"), n.data = JSON.parse(t.getAttribute("data-price-output")), l.push(n)
			}
...
function ()
{
	...
	const e = document.getElementsByClassName("smooth-scroll"),
		t = (e, n, i, s, a) =>
		{
			...
			window.scroll(0, a + s * r), l < i && window.requestAnimationFrame(n =>
			{
...
function ()
{
	...
	function t(e)
	{
		...
		document.getElementById(t).classList.add("ty"), e.classList.add("ty")
	}
	if (e.length > 0)
		for (let n = 0; n < e.length; n++)
		{
			...
			i.addEventListener("click", function (n)
			{
				...
				{
					for (let t = 0; t < e.length; t++) e[t].classList.remove("ty");
					for (let e = 0; e < i.length; e++) i[e].classList.remove("ty");
					t(this)
				}
